...
**cubrid-lab** is an independent open-source organization building modern
client tooling for [CUBRID](https://www.cubrid.org/), the Apache-2.0
licensed open-source RDBMS with strong adoption in the Korean public
sector.
...
- **Correctness over speed of shipping** — every change passes live-database
  CI matrices and coverage floors before merge.
- **Runnable truth** — claims are backed by runnable examples and
  reproducible benchmarks, not marketing copy.
- **Accessibility** — bilingual documentation (English canonical, Korean
  maintained) because a large share of CUBRID users work in Korean.
